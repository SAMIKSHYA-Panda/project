...
public class InsuranceCompanyController {
	...
	public List<InsuranceCompany> findAllCompany() {
		System.out.println("findAllCompany is called");
		return companyDao.findAllCompany();
	}
	...
	public String deleteCompanyById(String companyId) {
		...
		if (company == null) {
			context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
					"Company not found with ID: " + companyId, null));
			return "not found";
		}
 
		System.out.println("Deleting Company: " + company);
 
		companyDao.deleteCompanyById(company.getCompanyId()); // ✅ FIXED
		showSuccessMessage = "Company deleted successfully!";
 
		context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
				"Company deleted successfully!", null));
	...
	public String updateCompanyById(InsuranceCompany company) {
		...
		if (validateCompanyWithFacesMessage(company)) {
			...
			context.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Validation failed!", null));
		}
	...
	public boolean validateCompanyWithFacesMessage(InsuranceCompany company) {
		...
		if (company.getName() == null || company.getName().trim().isEmpty()) {
			context.addMessage("companyForm:name",
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Company name is required.", null));
			isValid = false;
		} else if (company.getName().trim().length() < 4) {
			context.addMessage("companyForm:name",
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Name must be at least 4 characters.", null));
			isValid = false;
		}
		...
		if (company.getContactEmail() == null || company.getContactEmail().trim().isEmpty()) {
			context.addMessage("companyForm:email",
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Email is required.", null));
			isValid = false;
		} else if (!company.getContactEmail().trim().matches("^[A-Za-z]{4}[A-Za-z0-9._%+-]*@gmail\\.com$")) {
			context.addMessage("companyForm:email",
					new FacesMessage(FacesMessage.SEVERITY_ERROR,
							"Email must start with 4 letters and end with '@gmail.com'.", null));
			isValid = false;
		}
		...
		if (company.getHeadOffice() == null || company.getHeadOffice().trim().isEmpty()) {
			...
			context.addMessage("companyForm:headOffice",
					new FacesMessage(FacesMessage.SEVERITY_ERROR,
							"Head Office name must be at least 4 characters.", null));
			isValid = false;
		}
		...
		if (company.getLogoUrl() == null || company.getLogoUrl().trim().isEmpty()) {
			...
			if (logoUrl.length() < 4) {
				context.addMessage("companyForm:logoUrl",
						new FacesMessage(FacesMessage.SEVERITY_ERROR,
								"Logo URL must be at least 4 characters.", null));
				isValid = false;
			} else if (!logoUrl.matches("^([\\w\\-]+/)?[\\w\\-]+\\.(jpg|jpeg|png|gif)$")) {
				context.addMessage("companyForm:logoUrl",
						new FacesMessage(FacesMessage.SEVERITY_ERROR,
								"Logo must be a valid image file (jpg/jpeg/png/gif).", null));
				isValid = false;
			}
		}
