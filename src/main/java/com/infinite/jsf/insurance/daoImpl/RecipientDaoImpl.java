...
public class RecipientDaoImpl implements RecipientDao {
	...
	public void saveRecipient(Recipient recipient) {
		 Transaction tx = null;
	        ...
	        try {
	            session = SessionHelper.getSessionFactory().openSession(); // Hibernate 3.6
	            tx = session.beginTransaction();
	            recipient.setCreatedAt(new java.util.Date()); // set timestamp
	            session.save(recipient);
	            tx.commit();
	        } catch (Exception e) {
	...
	@Override
	public Recipient getRecipientById(String hId) {
		 Session session = null;
	        try {
	            session = SessionHelper.getSessionFactory().openSession(); // Hibernate 3.6
